...
function createBarrageItemDom <T extends BarrageType>(type: T, data: BarrageDataByType<T>, index: number) {
  const dom = document.createElement('div')
  dom.style.transform = 'translate3d(-100%, -100%, 0)'

  if (type === BARRAGE_TYPE_NOBLE) {
    dom.className = clsx(BASE_BARRAGE_ITEM_CLASSNAME, 'px-4 py-1')
    dom.innerHTML = `<div class="flex items-center bg-red-300 rounded-[50px] px-2 py-1">
      <div class="rounded-full bg-red-500 w-10 h-10 mr-2"></div>
      <div class="text-black">${index}. BARRAGE_TYPE_NOBLE(貴族)</div>
    </div>`
  } else if (type === BARRAGE_TYPE_BET_WIN) {
    dom.className = clsx(BASE_BARRAGE_ITEM_CLASSNAME, 'px-4 py-1')
    dom.innerHTML = `<div class="bg-green-300 rounded-[50px] px-3 py-1 text-[12px] text-black">${index}. BARRAGE_TYPE_NOBLE(投注中獎)</div>`
  } else if (type === BARRAGE_TYPE_STEED) {
    dom.className = clsx(BASE_BARRAGE_ITEM_CLASSNAME, 'px-4 py-1')
    dom.innerHTML = `<div class="bg-yellow-300 rounded-[50px] px-2 py-1 text-black">${index}. BARRAGE_TYPE_NOBLE(坐騎)</div>`
  }
...
export function BarrageList (
  ...
  const itemInfoListRef = useRef<BarragePrivateItemInfo[]>([])

  useMittOn(appendMittType, (ev) => {
    itemInfoListRef.current.push({
      ...ev,
      dom: null,
      x: 0,
      y: 0,
...
type BarrageMoveParams = {
  wrap: string
  itemInfoListRef: MutableRefObject<BarragePrivateItemInfo[]>
  offsetY?: number
  offsetHeight?: number
}
function createMoveAllBarrageItemAnimationFrame (
  ...
  function unmount () {
    ...
    for (let i = 0; i < itemInfoListRef.current.length; i++) {
      itemInfoListRef.current[i].dom?.remove()
    }
  }
  ...
  function moveAllBarrageItem () {
    ...
    const itemInfoList = itemInfoListRef.current
